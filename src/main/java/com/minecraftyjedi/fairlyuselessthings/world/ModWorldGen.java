...
public class ModWorldGen implements IWorldGenerator {
	...
	private void generateOverworld(Random random, int chunkX, int chunkZ, World world, IChunkGenerator chunkGenerator,
		...
		if (random.nextInt(10) > 8) {
			this.generateAirOre(ModBlocks.HALLOW_ORE_BLOCK.getDefaultState(), world, random, chunkX * 16, chunkZ * 16,
					150, 151, random.nextInt(3) + 1, 1);
		} else {
			;
		}
	...
	private void generateAirOre(IBlockState ore, World world, Random random, int x, int z, int minY, int maxY, int size,
			int chances) {
		...
		for (int i = 0; i < chances; i++) {
			...
			WorldGenAirOre generator = new WorldGenAirOre(ore, size);
			generator.generate(world, random, pos);
		}
	}
